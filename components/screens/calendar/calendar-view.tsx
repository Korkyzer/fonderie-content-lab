  ...
  const entriesByDateKey = useMemo(() => {
    ...
    filteredEntries.forEach((entry) => {
      const entryDate = entry.dateIso
        ...
        : isReferenceMonth
          ? new Date(MONTH_YEAR, MONTH_INDEX, entry.day, 12)
          : null;
      if (!entryDate) return;
      const key = dateKeyOf(entryDate);
          ...
          <div className="overflow-hidden rounded-md border border-ink/8 bg-cream shadow-card">
            ...
            <div className="grid grid-cols-7">
              {monthCells.map((cell, index) => {
                ...
                const isToday = Boolean(cellDate && isSameDay(cellDate, new Date()));
                const aiText = !isPad ? aiByDay.get(cell.num) : undefined;
                const entries = cellDate
                  ? entriesByDateKey.get(dateKeyOf(cellDate)) ?? []
                  : [];
        ...
        <div className="overflow-hidden rounded-md border border-ink/8 bg-cream shadow-card">
          ...
          <div className="grid grid-cols-7">
            {weekDays.map((day) => {
              ...
              const entries = entriesByDateKey.get(dateKeyOf(day)) ?? [];
